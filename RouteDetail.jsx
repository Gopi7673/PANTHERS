...
const RouteDetails = () => {
  ...
  const downloadSchedule = () => {
    ...
    doc.setProperties({
      title: `Route Schedule ${id}`,
      author: 'Route Management System',
      subject: 'Route Details',
      keywords: 'route, schedule, transportation'
    });
  
    // Header styling
    doc.setFontSize(20);
    doc.setFont('helvetica', 'bold');
    doc.text('Route Schedule', 105, 20, { align: 'center' });
    
    // Add company logo placeholder
    // doc.addImage('/api/placeholder/50/50', 'PNG', 20, 10, 30, 30);
    ...
    const details = [
      ...
      ['Estimated Duration:', `${route.estimated_duration}`]
    ];
    ...
    details.forEach((row, index) => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(row[1], 70, startY + (index * lineHeight));
    });
    ...
    doc.text(`Generated on: ${timestamp}`, 20, pageHeight - 20);
    doc.text('Page 1 of 1', 190, pageHeight - 20, { align: 'right' });
    
    // Save the document
    doc.save(`route_${id}_schedule.pdf`);
  };
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <DetailCard
                icon={<DollarSign />}
                title="Fare"
                value={route.base_fare ? `$${route.base_fare}` : 'Confirmed On Schedule'}
              />
              <DetailCard
                icon={<Users />}
                title="Available Seats"
                value={route.available_seats || 'Confirmed On Schedule'}
              />
              <DetailCard
                icon={<Box />}
                title="Cargo Space"
                value={route.cargo_capacity || 'Available'}
              />
            </div>
          </CardContent>
        </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <button
                    className="w-full py-2 px-4 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
